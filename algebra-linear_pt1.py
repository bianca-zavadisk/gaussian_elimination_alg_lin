        ...
        for j in range(m):
            if i == j:
                if u_matrix.entries[i][j] == 0:
                    ...
                    #Row_exchange
                    p_matrix = permutation(n, i, pivot_row)*p_matrix
                    u_matrix = p_matrix*u_matrix
                    
                    l_matrix = lower_triangular(n, l_matrix, p_matrix)
        
                pivot = u_matrix.entries[i][j]          

        #Row operations
        for k in range(i+1, n):
            if not pivot: #Avoid divison by zero
                for k in range(i+1, m):
                    if u_matrix.entries[i][k]:
                        pivot = u_matrix.entries[i][k]
                        break      
            scale = (u_matrix.entries[k][i] / pivot)*(-1)
            if scale:
                op_matrix = lines_operation(n, k, i, scale)
                u_matrix = op_matrix*u_matrix
                l_matrix = lower_triangular(n, l_matrix, lines_operation(n, k, i, scale, True))
                
        #Row scale to get pivot = 1
        for i in range(n):
            for j in range(m):
                if i == j:
                    pivot = u_matrix.entries[i][j]
            ...
            if pivot != 1 and pivot:
                ...
                u_matrix = op_matrix*u_matrix
                l_matrix = lower_triangular(n, l_matrix, line_scale(n, i, 1/pivot, True))
                
            for k in range(1, n-i): #Divides linearly dependent rows
                if u_matrix.entries[i] == u_matrix.entries[i+k]:
                    op_matrix = lines_operation(n, i, i+k, -1)
                    u_matrix = op_matrix*u_matrix
                    l_matrix = lower_triangular(n, l_matrix, lines_operation(n, i, i+k, -1, True))
